...
import { ArrowDown, ArrowRight, Check, Leaf, LockKeyhole, MapPin, NotebookPen, Paperclip, Sparkles, Volume2, VolumeX } from 'lucide-react'
import { SITE_BUILD, bonuses, chapters, freeTasks } from './content'
import './styles.css'
...
function App(){
  ...
  const saveHabitField = (field,value)=>{
    setHabitDraft(prev=>({...prev,[field]:value}))
    setHabitDirty(true)
  }
  ...
  const toggleHabitDay = (i)=>{
    ...
    const start = dateFromISO(habitDraft.startDate)
  ...
  const resetHabit = ()=>{
    ...
    setHabitDraft(empty)
    localStorage.setItem(HABIT_KEY, JSON.stringify(empty))
    setHabitDirty(false)
  }
  ...
  const startAmbient = async(mode)=>{
    try{
      ...
      if(!Ctx){ setAmbientError('В этом браузере звук не поддерживается.'); return }
      const ctx = new Ctx()
      ...
      if(mode==='rain'){
        filter.type='highpass'; filter.frequency.value=1700; gain.gain.value=.34
      }
      if(mode==='waves'){
        filter.type='lowpass'; filter.frequency.value=520; gain.gain.value=.38
        const swell = ctx.createOscillator(); swell.frequency.value=.17
        const depth = ctx.createGain(); depth.gain.value=.12
        swell.connect(depth); depth.connect(gain.gain); swell.start()
        audioNodesRef.current.push(swell,depth)
      }
      if(mode==='forest'){
        filter.type='bandpass'; filter.frequency.value=850; gain.gain.value=.16
        const bird = ctx.createOscillator(); bird.type='sine'; bird.frequency.value=1050
        const birdGain = ctx.createGain(); birdGain.gain.value=.006
        const lfo = ctx.createOscillator(); lfo.frequency.value=.23
        const lfoDepth = ctx.createGain(); lfoDepth.gain.value=190
        lfo.connect(lfoDepth); lfoDepth.connect(bird.frequency); bird.connect(birdGain); birdGain.connect(master)
        bird.start(); lfo.start(); audioNodesRef.current.push(bird,birdGain,lfo,lfoDepth)
      }

      noise.connect(filter); filter.connect(gain); gain.connect(master); noise.start()
      audioNodesRef.current.push(noise,filter,gain)
      setAmbientMode(mode)
    }catch{
      setAmbientError('Не получилось включить звук. Попробуй ещё раз.')
    }
  }
